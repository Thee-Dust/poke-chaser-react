  ...
  return (
    <div className="binder-tile">
      ...
      <div className="binder-tile__content">
        <div className="binder-tile__info">
          <span className="binder-tile__name">{binder.name}</span>
          <span className="binder-tile__meta">
            {binder.page_count} {binder.page_count === 1 ? 'page' : 'pages'} · {binder.rows}×{binder.cols}
          </span>
        </div>
        <span className="binder-tile__count">
          {binder.card_count} {binder.card_count === 1 ? 'card' : 'cards'}
        </span>
      </div>
    </div>
  )
